...
export class AvoidsLinksRouter extends go.Router {
  ...
  collectSegments(links, coll) {
    ...
    const skipBounds =
      coll instanceof go.Diagram && links.count === coll.links.count;
    // add segments of links in the "invalid" links Set
    for (const l of links) {
      if (
        !this._ignoreContainingGroups &&
        !(
          (isDiagram && l.containingGroup === null) ||
          l.containingGroup === coll
        )
      )
        continue;
    ...
    if (coll && enclosingRect !== null && !skipBounds) {
      for (const l of this.diagram.findPartsIn(enclosingRect, true)) {
        ...
        while (i < l.pointsCount - 1) {
          ...
          for (const line of this._allsegs) {
            if (
              Math.abs(this._coord(line.first()) - coord) <
                this._epsilonDistance &&
              line.first().vertical === vertical
            ) {
              const seginfo = this._allocSegInfo();
        ...
        while (found) {
          ...
          for (const otherseg of newline) {
            for (const seginfo of line) {
              const minI = Math.min(
                this._columnStart(seginfo),
                this._columnEnd(seginfo)
              );
              const maxI = Math.max(
                this._columnStart(seginfo),
                this._columnEnd(seginfo)
  ...
  adjustOverlaps(positions) {
    ...
    for (const line of gridlines) {
      ...
      if (this._avoidsNodes && maxlayer > 0) {
        ...
        const minCoord =
          this._coord(gridline[0]) - (maxlayer * this._linkSpacing) / 2; // x if vertical
        const maxCoord =
          this._coord(gridline[0]) + (maxlayer * this._linkSpacing) / 2;
        ...
        if (vertical) {
          if (
            ...
            realSpacing = Math.min(
              this._linkSpacing,
              (2 * availSpace) / (1 + maxlayer)
            );
          }
        } else {
          if (
      ...
      for (let i = 0; i < gridline.length; i++) {
        ...
        for (let j = seg.indexStart; j <= seg.indexEnd; j++) {
          if (vertical) {
            seg.link.setPoint(
              j,
              new go.Point(newcoord, seg.link.getPoint(j).y)
            );
          } else {
            seg.link.setPoint(
              j,
              new go.Point(seg.link.getPoint(j).x, newcoord)
            );
          }
        }
